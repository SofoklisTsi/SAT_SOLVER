...
# Set the filename, solver type, heuristic and the boolean parameters. THOSE ARE THE ONLY VARIABLES THAT NEED TO BE CHANGED.
filename = "../sat_solver/DATA/DPLL_TESTS/TESTS/twl.cnf"
solver_type = "dpll" # or "cdcl"
heuristic = "default" # or "dlcs" or "dlis" or "moms"
cutting_method = "1UIP"
twl = False # or True
true_twl = False # or True
use_logger = True # or False
...
if solver_type == "dpll":
    solver = solver_function(clauses_model=clauses_model, use_logger=use_logger, heuristic=heuristic, twl=twl, true_twl=true_twl)
else:
    solver = solver_function(clauses_model=clauses_model, use_logger=use_logger, heuristic=heuristic, twl=twl, true_twl=true_twl, cutting_method=cutting_method)
